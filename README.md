...
- terminal resize and interrupt/terminate/kill signals;
...
Podman gets one additional capability probe. When Bubblewrap itself works, CodexBridge runs a bounded, non-mutating `podman info --format json` probe inside the same sandbox shape. If that succeeds, actual Podman commands remain inside Bubblewrap. If it fails or times out, Podman uses the native backend only when unsandboxed execution is allowed; otherwise the command fails with `SANDBOX_UNAVAILABLE`. Native fallback only selects where the command runs; it does not grant extra container-engine privileges. A host may therefore allow `podman info` while a rootless `podman run` still fails on namespace or mount permissions. CodexBridge does not automatically retry such a command with elevated privileges. If a project environment requires rootful Podman, its instructions should invoke `sudo podman ...` explicitly rather than relying on a shell alias, because `exec_command` runs a non-interactive shell and aliases/profile setup are not part of its contract. CodexBridge does not prescribe what Podman is used for; build, run, dependency, and runtime-build workflows belong in the project's `AGENTS.md`.
