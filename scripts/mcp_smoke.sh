...
# Once bound, omitting the previous ref is rejected rather than silently creating
# an ambiguous additional turn.
call_tool chatgpt_turn_init 47 '{}' "$run_root/project-missing-parent.json"
jq -e '.result.isError == true and .result.structuredContent == null and (.result.content[0].text | startswith("PREVIOUS_TURN_REF_REQUIRED:"))' "$run_root/project-missing-parent.json" >/dev/null
...
jq -e '.result.isError == true and .result.structuredContent == null and (.result.content[0].text | startswith("TURN_REF_NOT_FOUND:"))' "$run_root/cross-subject-ref.json" >/dev/null
